@extends('layouts.app')
@section('title', __('admin.teachers.edit'))

@section('content')
<div style="max-width: 640px; margin: 0 auto; padding: 16px;">
    <div style="background: #fff; border-radius: 18px; border:1px solid #eef0f4; box-shadow: 0 8px 25px rgba(0,0,0,0.06); padding: 28px; overflow:hidden;">
        <h2 style="color: #0a1f4f; text-align: center; margin-bottom: 20px; font-weight: 800; font-size: 22px;">
            {{ __('admin.teachers.edit') }}
        </h2>

        <form action="{{ route('admin.guru.update', $guru->id) }}" method="POST" style="display:grid; gap:14px; width:100%;">
            @csrf
            @method('PUT')
            ...
            <div class="fi">
                ...
                <input class="ip" type="text" name="nama" value="{{ old('nama', $guru->nama) }}" required>
            </div>

            {{-- Jabatan --}}
            <div class="fi">
                <label class="lb">{{ __('admin.teachers.form.position') }}</label>
                <input class="ip" type="text" name="jabatan" value="{{ old('jabatan', $guru->jabatan) }}">
            </div>
